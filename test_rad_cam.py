...
class TestRadCam(unittest.TestCase):
    ...
    def test_calculate_diffs_binary_pred(self):
        pred = np.array([1.])
        altered_pred = np.array([[.4], [.3], [.2], [2.1]])
        output = np.array([[.6], [.7], [.8], [1.1]])
        np.testing.assert_array_almost_equal(
            calculate_diffs(pred, altered_pred),
            output, decimal=10)

    def test_calculate_diffs_multiple_pred(self):
        pred = np.array([.9, .3])
        altered_pred = np.array([[.4, .5], [.3, .3]])
        output = np.array([[.5, .2], [.6, 0.]])
        np.testing.assert_array_almost_equal(
            calculate_diffs(pred, altered_pred),
            output, decimal=10)
    ...
    def test_conversion_of_different_pred_types_single(self):
        for model in [
                MockNumpyOutputModel(),
                MockTorchOutputModel()]:
            radcam = RadCam(
                model,
                image_width=self.input_rgb_image.size[0],
                image_height=self.input_rgb_image.size[1],
